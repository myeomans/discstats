...
for (l in 1:length(Bees)){
  l<-1
  filterc<-(Play$homeOnOffense==1&Play$receiverId>0)
  ...
  for (k in 1:nrow(Play)){
    ...
    if (filtert[k]&(Play[k,]$pickup==1)){
      ...
      #MUST FIRGURE OUT THROWBIN FOR PICKUPS!!!
  ...
  namer<-Bees[[l]]@name
  print(namer)
 # png(paste(namer, "dot.png"), width=1000, height=1000)
  ...
  points(x=((1:13)+0.5), y=rep(16.5,13), pch=16,
         cex=scaler(Bees[[l]]@drops, Bees[[l]]@Opos), col="dark red")
  ...
  points(x=rep((14.7),13), y=((1:13)+0.5), pch=16,
         cex=scaler(Bees[[l]]@pupCs, Bees[[l]]@Opos), col="dark blue", xpd=TRUE) 
  points(x=rep((14.7),13), y=((1:13)+0.5), pch=16,
         cex=scaler(Bees[[l]]@pupOs, Bees[[l]]@Opos), col="red", xpd=TRUE) 
  ...
  for (i in 1:13){
    points(x=((1:13)+0.5), y=rep((i+0.5),13), pch=20,
           cex=scaler(Bees[[l]]@ThrOs[,i], Bees[[l]]@Opos), 
           col="red") 
  }
  ##########LEGEND AND POSS COUNT#############
  points(x=11.95, y=11, pch=16,
         cex=scaler(1, 100), col="black")
  text(13.2, 11, "= 1%", cex=2.5, adj=0.5, xpd=TRUE)
